...
c_types <- c(tourney_id    = "c", tourney_name = "c", surface   = "c", draw_size = "i",
             tourney_level = "c", tourney_date = "c", match_num = "c",
              
             winner_id   = "c", winner_seed = "i", winner_entry = "c", winner_name = "c",
             winner_hand = "c", winner_ht   = "d", winner_ioc   = "c", winner_age  = "d",
              
             loser_id   = "c", loser_seed = "i", loser_entry = "c", loser_name = "c",
             loser_hand = "c", loser_ht   = "d", loser_ioc   = "c", loser_age  = "d",
              
             score = "c", best_of = "i", round = "c", minutes = "i",
              
             w_ace    = "i", w_df    = "i", w_svpt    = "i", w_1stIn   = "i", w_1stWon = "i",
             w_2ndWon = "i", w_SvGms = "i", w_bpSaved = "i", w_bpFaced = "i",
              
             l_ace    = "i", l_df    = "i", l_svpt    = "i", l_1stIn   = "i", l_1stWon = "i",
             l_2ndWon = "i", l_SvGms = "i", l_bpSaved = "i", l_bpFaced = "i",
              
             winner_rank = "i", winner_rank_points = "i",
             loser_rank  = "i", loser_rank_points  = "i")

tennis_import <- read_csv("https://raw.githubusercontent.com/JeffSackmann/tennis_atp/master/atp_matches_1968.csv",
  col_types = c_types) %>%
  mutate(source = "atp_matches_1968.csv")

for (i in 1969:2021) {
  filename <- paste0("atp_matches_", i, ".csv")
  link <- paste0("https://raw.githubusercontent.com/JeffSackmann/tennis_atp/master/", filename)
  
  tennis_import <- tennis_import %>%
    bind_rows(
      read_csv(link, col_types = c_types) %>%
...
total_gs_wins <- gs_wins %>%
  slice_max(tourney_date)

# get GS winners first match
gs_winners <- gs_wins %>%
  distinct(player_id, player_name)

gs_winners_matches <- atp_tennis %>%
  filter(tourney_level == "G", round != "F") %>%
  select(tourney_id,
         tourney_name,
         surface,
...
gs_total_wins <- gs_winners_matches %>%
...
ggplot() +
  # <10 wins
  geom_line(data = filter(gs_winners_matches, !(!!sym(var))),
